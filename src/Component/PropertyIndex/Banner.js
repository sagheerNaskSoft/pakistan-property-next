...
function Banner() {
    const { citiesList, currentCity, propertyType, locationList, objectToQueryString, buildSearchPayloadCPropertyIndex } = useAuth()
    ...
    const handleNavigate = async () => {
        let newData = await buildSearchPayloadCPropertyIndex(filterData)
        const queryString = objectToQueryString(newData);
    ...
    }, [filterData, buildSearchPayloadCPropertyIndex, objectToQueryString, navigate]);
    ...
    useEffect(() => {
        if (propertyType?.length) {
            setActiveTab(propertyType[0])
        }
    }, [propertyType])
    
    return (
        <>
            <div className="property-index-banner">
                <div className="main-container">
                    <div className="text-box">
                        <div className="heading">Track Pakistan’s Real Estate Growth</div>
                        <div className="para">Compare buying and rental rates across different cities and localities with verified data.</div>
                        <div className="input-section" ref={searchAreaRef}>
                            <div className="button-box">
                                {propertyType?.map((item) => {
                                    if (item?.slug !== "lease") {
                                        return (
                                            <div className={`button ${activeTab?.slug === item?.slug ? "active" : ""}`} onClick={() => {
                                                setActiveTab(item)
                                                setFilterData({ ...filterData, purpose: item?.id })
                                            }}>{item?.name === "Sell" ? "Buy" : item?.name}</div>
                                        )
                                    }
