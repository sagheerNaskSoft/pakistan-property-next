...
import img from '../../Asset/BLog/ImageWithFallback (1).png'
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../Context/ContextProvider';
import { FallbackImage } from '../Metiral/FallbackImage';
import ad from '../../Asset/Ads/3.gif'
function Bloglisting({ popular_news, blogList, categoryList, pagination, filterData, setFilterData, blogSlider, setBlogSlider }) {
    const [activeCategory, setActiveCategory] = useState("All Categories");
    const navigate = useNavigate()
    const { objectToQueryString, getAd, adsClick, adsImpression } = useAuth()
    ...
    const handlePageChange = async (pageAction) => {
        ...
        const queryString = objectToQueryString({ ...filterData, current_page: goPage });
        navigate(`/all-news?${queryString}`);
    }
    useEffect(() => {
        if (filterData?.category) {
            setActiveCategory(filterData?.category)
        } else {
        ...
        <>
            <div className='blog-listing'>
                <div className='row mx-0 mb-4'>
                    <div className='col-lg-3 ps-0 pe-lg-3 pe-0 order-lg-1 order-2'>

                        <div className='blog-side-section h-100'>
                            <div className='row'>
                                    ...
                                    <div className={`blog-categories ${blogSlider ? 'blog-slider-categories' : ''}`}>
                                        ...
                                        <ul>
                                            ...
                                            >
                                                ...
                                                <span className="count">{categoryList?.reduce((acc, cat) => acc + cat.news_count, 0)}</span>
                                            </li>
                                            {categoryList
                                                ?.filter(cat => Number(cat.news_count) > 0)
                                                .map((cat) => (
                                                    <li
                                                        key={cat.name}
                                                        onClick={() => {
                                ...
                                <div className='col-lg-12'>
                                    <h3 className='d-sm-none d-block'>Popular News</h3>
                                    <div className='popular-blogs'>
                                        <h3 className='d-none d-sm-block'>Popular News</h3>
                                        {popular_news?.map((item, index) => {
                                            return (
                                                <div
                                                    key={`popular-news-${item?.id || item?.slug || index}`}
                                                    onClick={() => {
                                        ...
                                        <div className="news-inputt-section">
                                            ...
                                            >
                                                <input
                                                    ...
                                                    className="news-inputt"
                                                />

                                            </form>
                                            <button type="submit" className="news-sub-btn">Subscribe</button>
                                        </div>
                            ...
                            {/* <div className="global-ad-box w-100 mt-3 d-md-flex d-none" style={{ height: "25.2%" , minHeight:'260px' }}>
                                Advertisement Space
                            </div> */}
                    ...
                    <div className="col-lg-9 order-lg-2 order-1 ps-lg-0 px-0">
                        <div className='blog-card-data'>
                            <h1>All News</h1>
                            <h5 style={{ cursor: "pointer" }} onClick={() => {
                                navigate('/all-news')
                            }}>View All <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <div className='blog-grid'>
                            {getMixedContent.map((item) => {
                                if (item.type === 'blog') {
                                    return <BlogCard key={item.data?.id || item.index} route={"news"} data={item.data} />;
                        ...
                        <div className="card-section">
                            {pagination?.total_pages > 1 ? <div className="col-12 mt-4">
                                <div className="row w-100 m-0 align-items-center">
                                    <div className="col-md-6 mb-sm-0 mb-4 col-sm-6 col-12 order-sm-first order-last p-0">
                                    ...
                                    <div className="col-md-6 d-sm-block d-none col-4 p-0">
