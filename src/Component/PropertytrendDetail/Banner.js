...
function Banner({ filterData, setFilterData, open, setOpen }) {
    const { citiesList, locationList, propertyType, objectToQueryString, buildSearchPayloadCPropertyTrend } = useAuth()
    ...
    const handleSearch = useCallback(async () => {
        // Check if there's data to search (purpose, city, or location)
        const hasData = filterData?.purpose || 
                       filterData?.city ||
                       filterData?.location;
    ...
    return (
        <>
            <div className={`property-index-deatail property-index-detail-banner ${open ? 'open' : ''}`}>
