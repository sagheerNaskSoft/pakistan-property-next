...
function Banner({ filter, pagination, categoryList, blogList, blogSlider, setBlogSlider }) {
    ...
    return (
        <div className="all-blogs-section">
            ...
            <div className={`main-container ${blogSlider ? "blog-slider-container" : ""}`}>
                <div className="all-blogs-banner-details">
                    <span className="d-none d-sm-block"><BreadCrumb items={['Home', "News", "All News"]} /></span>
                    <h4>OUR NEWS</h4>
                    <h1>Find our all news from here</h1>
                    <h4>Our news are written from very research research and well known writers writers so that  we can<br /> provide you the best blogs and articles articles for you to read them all along</h4>
                </div>
                {/* <h6>All News</h6> */}
                <div className='blog-grid mt-4'>
                    {blogList?.map((item, index) => {
                        return (
                            <BlogCard key={`all-news-blog-${item?.id || item?.slug || index}`} route={"news"} data={item} />
                        )
                    })}
                </div>
                <div style={{ background: "none", borderBottom: "1px solid", paddingBottom: "30px" }} className="card-section">
                    {pagination?.total_pages > 1 ? <div className="col-12 mt-4">
                        <div className="row w-100 m-0 align-items-center position-relative">
                            ...
                            <div className="position-absolute end-0 p-0" style={{ top: '50%', transform: 'translateY(-50%)' }}>
                                            ...
                                            showCard ?
                                                <>
                                                    <div className="input-drop-menu">
                                                        {
                                                            num?.map((item) => (
                                                                <div key={`per-page-${item}`} className={filter?.per_page === item ? "title active" : "title"} onClick={async () => {
                                                                    setShowCard(false)
