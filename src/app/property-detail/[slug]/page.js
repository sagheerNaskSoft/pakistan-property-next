...
export async function generateMetadata({ params }) {
  ...
  const imageCandidate = Array.isArray(property?.property_images)
    ? property.property_images[0]
    : null;
  const imagePath = typeof imageCandidate === 'string' ? imageCandidate : imageCandidate?.image || imageCandidate?.url;
  const image = toAbsoluteUrl(imagePath || FALLBACK_IMAGE, siteUrl);
  const url = `${siteUrl}/property-detail/${slug || ''}`;
