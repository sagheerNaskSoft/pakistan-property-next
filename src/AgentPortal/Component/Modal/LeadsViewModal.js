...
function LeadsViewModal({ data, setOpen, open }) {
    const handleClose = () => {
        setOpen(false)
    }
    return (
        ...
        >
            <Box sx={style} className='leads-view-modal'>
                <div className='apply_quota_modal leads_modal px-0 pb-0 global-modal-base-color'>
                   ...
                   <div className='header_modal global-mobile-menu-header' >
                    <div className="d-sm-none d-flex">
                        <h5 className='heading mb-0 pb-0' style={{borderImage:'unset' , border:'none'}}>Lead For Property ID: {data?.property_id}</h5>
                    </div>
                        <div className="close-button" style={{position : window.innerWidth <= 576 ? 'unset' : ''}} onClick={handleClose}>
                        ...
                        <h5 className='d-sm-flex d-none'>Lead For Property ID: {data?.property_id}</h5>
                        
                    </div>
                    <div className='lead_content'>
                        <div className='row mx-0 mt-sm-2 mt-3 mb-3' style={{gap:" 10px 0"}}>
                            <div className='col-12 px-0 d-flex align-items-center' style={{gap:"10px"}}>
                            <h6>Name:</h6>
                            ...
                            <div className='col-12 p-0 d-flex flex-column' style={{gap:"8px"}}>
                            <h6>Message:</h6>
                            <p className='message-text'>{data?.message ? data.message.charAt(0).toUpperCase() + data.message.slice(1) : data?.message}</p>
                            </div>
                        </div>
                    </div>
                    </div>
